...
/// Locate a real cross glibc `ld-linux-riscv64-lp64d.so.1` in the Nix store (the interpreter
/// to embed in the dynexe). Returns an allocated absolute path, or null if none is found.
fn findCrossInterp(allocator: std.mem.Allocator) !?[]u8 {
    const proc = std.process.run(allocator, std.testing.io, .{
        .argv = &.{ "sh", "-c", "find /nix/store -maxdepth 3 -name ld-linux-riscv64-lp64d.so.1 2>/dev/null | head -1" },
    }) catch return null;
