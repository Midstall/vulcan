//! End-to-end validation harness for the microarch optimizer pipeline. This is the
//! correctness oracle for the whole pipeline: it runs the FULL pipeline, `microarch.optimize`
//! (unroll, vectorize, prefetch, schedule, in that order), followed by the model-aware backend
//! compile that fires the loop-header alignment hook, against a plain compile of an
//! identically-built, untouched copy of the same function. The test JITs both in-process on this
//! host and calls them with a spread of inputs. Any divergence is a miscompile somewhere in the
//! pipeline, never a value to fudge.
//!
...
//! calling the optimizer" (today's behavior) against "the whole pipeline wired up for ampere-altra".
//!
//! The test also measures cycle counts ON LINUX, via perf_event_open (PERF_COUNT_HW_CPU_CYCLES).
//! That path is Linux-only. On any other OS the whole perf section is comptime-eliminated, and only
//! the correctness differential runs. The minimal open/reset/read sequence mirrors the OS-guarded
//! pattern in tools/uarch-bench.zig. This file repeats that pattern rather than sharing it, because
//! that tool and this test live in different modules. Cycle counts are NOT logged: printing from a
//! unit test makes the test runner surface spurious-looking output, and a human-readable cycle
//! report belongs in the dedicated benchmark (vulcan-uarch-bench). Here the test only checks them
//! against a loose sanity bound (tuned less than 10x baseline, see checkBound), to catch a gross
//! pipeline regression, not to assert "never slower". Cycle counts jitter in a unit-test context,
//! and a strict no-slower gate belongs in that benchmark. Correctness, the JIT differential, is the
//! hard assert.
//!
//! The correctness differential needs the aarch64 JIT backend, so it skips on other arches. The
//! cycle measurement additionally needs Linux, so it does not run on non-Linux aarch64. Everything
//! skips cleanly, so `zig build test` stays green on every host. On this dev Altra box it runs for
//! real.
...
/// The model this harness tunes for: the dev box (Ampere Altra / Neoverse N1). This is a wide
/// out-of-order part with a non-trivial fetch_align, so both the IR passes and the alignment
/// hook have real work to consider.
fn ampere() *const opt.microarch.Model {
    return opt.microarch.modelFor(.@"ampere-altra");
}

/// Kernel 1: a dependent-mul chain, straight-line, with no loop. It sets `r = x`, then repeats
/// `r = r*x` 5 times, so `f(x) = x^6`. Every multiply depends on the previous one. This shape
/// exercises scheduling: arranging a dependency chain for the model's ports and latency. There is
/// nothing here for unroll, vectorize, or prefetch to do, because there is no loop and no
/// independent lanes. So this kernel also proves the pipeline is a safe, near no-op pass over
/// straight-line code, not just over loops.
...
/// Kernel 2: a counted sum-reduction loop. It sets s to 0, then for i from 0 up to n, adds i to s
/// each time, and returns s. It carries two values, i and s. The accumulator escapes the loop and
/// is read directly at the loop-exit block. This is the same loop-closed-SSA shape proven under
/// the unroller in unroll_differential.zig's `buildSum`, here widened to i64. It exercises unroll,
/// which exposes ILP across iterations, and schedule, together.
...
/// Kernel 3: a strided pointer-walking reduction over a real backing array. It sets s to 0 and p to
/// arr, then for i from 0 up to n, adds the value at p to s and advances p by 8 each iteration,
/// then returns s. `p` is a pointer-typed loop-carried value that steps by a constant 8-byte stride
/// and is dereferenced each iteration. This is the same affine-strided-load shape as
/// prefetch_differential.zig's `buildStridedSum`, reused here because it is exactly the real oracle
/// the prefetch pass needs: the pass reads genuine loop and load structure, not a hand-placed hint.
/// This kernel exercises prefetch, unroll, and schedule together, over real memory.
...
/// Enforce only a very generous gross-regression bound: tuned not more than 10x baseline. This is
/// enough to catch a genuinely broken pipeline, for example a pass gone wrong, or a bad alignment
/// computation making the tuned build wildly slower. It is not a precise performance claim. Cycle
/// counts jitter under parallel test load, from scheduler noise, cache and TLB state, and other
/// tests sharing the core. So this bound is intentionally loose. Do NOT tighten it to chase a
/// "faster" result here. A strict, repeatable no-regression gate belongs in a dedicated benchmark
/// (vulcan-uarch-bench), not a unit test. The measured values are deliberately NOT printed (see the
/// file header for why). Correctness, the JIT differential above, is the hard gate. This bound is
/// only a sanity check that nothing catastrophic happened.
...
test "microarch e2e: dependent mul chain (straight-line, scheduling)" {
    ...
    // untouched baseline computes, for every input. Any divergence is a miscompile in the
    // pipeline. Do not fudge the expected value. Find the pass that broke it instead.
