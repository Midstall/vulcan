...
pub const Linked = struct {
    ...
    data: []DataSym,
    /// Carried-forward `global_addr` (`.abs32`) relocations: byte offset (module-wide,
    /// into `code`) of each `mov rd, imm32`'s imm32 field, plus the symbol it targets.
    /// Resolved once the image is mapped (see `applyGlobalReloc`); intra-module `.call`
    /// relocs are always resolved above and never appear here.
    relocs: []isel.Reloc,
...
/// Patch a carried-forward `global_addr` (`.abs32`) relocation now that the symbol's
/// runtime address is known. `image` is the mapped image whose code section holds the
/// `mov`'s imm32 field at BYTE offset `r.offset` (matching `Linked.relocs`/`Linked.code`,
/// both byte-addressed); `site_addr` is the runtime address of that imm32 field itself
/// (unused - i386 has no rip-relative addressing, so no site-relative math like x86-64's
/// `.pcrel_lea`) and `target_addr` is the symbol's resolved runtime address. `image` is
/// `anytype` (rather than the concrete `jit_platform.MappedImage` instantiation) so this
/// stays independent of which arch's instruction-cache-sync hook parameterized it - see
/// aarch64/link.zig's `applyGlobalReloc` for the same rationale.
///
/// `mov rd, imm32` loads its operand verbatim (no PC-relative math): the low 32 bits of
/// `target_addr` are written little-endian into the 4 bytes at the site. (The JIT and
/// the qemu-i386 execution harness both place code well within the low 4GB, so the
/// truncation never drops real address bits.)
pub fn applyGlobalReloc(image: anytype, r: isel.Reloc, site_addr: usize, target_addr: usize) void {
    ...
    switch (r.kind) {
        .abs32 => {
            ...
            std.mem.writeInt(u32, bytes, @truncate(target_addr), .little);
        },
        .call => unreachable, // `.call` relocs are always resolved by `compileModule`, never carried forward
        .got_abs => unreachable, // `.got_abs` (GOT-indirect data import) is rejected by `compileModule`; the static image has no GOT
    }
}
...
pub fn compileModule(allocator: std.mem.Allocator, module: *const Module) Error!Linked {
    ...
    // carry every `.abs32` (`global_addr`) relocation forward, globalizing its offset
    // to a module-wide byte index. Its target is a runtime data address, unknown
    // until the image is mapped.
    ...
    for (compiled, 0..) |c, i| {
        for (c.relocs) |r| {
            ...
            switch (r.kind) {
                ...
                .abs32 => try global_relocs.append(allocator, .{ .offset = site, .symbol = r.symbol, .kind = .abs32 }),
                // A GOT-indirect data import (`via_got`) needs a synthesized GOT + GLOB_DAT,
                // which only the dynamic linker (`vulcan-link`) provides; the static in-memory
                // image has no GOT, so reject it cleanly rather than miscompiling.
                .got_abs => return error.Unsupported,
            }
        }
    }
