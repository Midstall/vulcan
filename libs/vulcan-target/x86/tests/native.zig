...
//! into W^X memory and calling it. This is valid only when the host is i386. On any other
//! host every case skips, since i386 machine code cannot run natively there. On an i386
//! host it also exercises the JIT (coherent_jit) path.
...
test "a rodata global read via global_addr runs in-process through native.jitModuleData (skips off i386)" {
    // Same module as the qemu test in qemu.zig, run through `harness.runModuleData`'s
    // native path (real only on an i386 host): `native.jitModuleData` maps code and rodata
    // for real and patches the `.abs32` reloc through the i386 `applyGlobalReloc`,
    // exercising the exact in-process JIT path production callers use (as
    // opposed to qemu.zig's static, harness-only reloc resolution).
...
test "codegen+disasm round-trip: integer add" {
    // Compile i386 and assert the disassembled listing (instruction selection and register
    // allocation). This runs no code, so it runs on any host. cdecl args come off the stack.
    const a = std.testing.allocator;
...
test "codegen+disasm round-trip: control flow (max via if/else)" {
    ...
    // no move. The else-arm copies y (ecx) into eax. Both paths return the larger of x and y.
    const a = std.testing.allocator;
    var func = ir.function.Function.init(a);
    defer func.deinit();
    const i32_t = try func.types.intern(.{ .int = .{ .signedness = .signed, .bits = 32 } });
...
test "codegen+disasm round-trip: alloca/store/load at displacement 0 (no qemu needed)" {
    // Smoke-checks the load/store lowering by disassembly, so it runs even where
    // qemu-i386 is unavailable: alloca an i32 slot, store the argument, load it back, return
    // it. Confirms the [reg+0] mov encoders are actually wired into `.alloca`,
    // `.load`, and `.store`, not just callable.
    const a = std.testing.allocator;
    var func = ir.function.Function.init(a);
    defer func.deinit();
    const i32_t = try func.types.intern(.{ .int = .{ .signedness = .signed, .bits = 32 } });
...
test "Wimmer path: add-two-args compiles and returns via eax" {
    ...
    // add. cdecl params come off the stack into the allocator-chosen homes. This is a structural
    // check only (a full execution differential lives elsewhere): assert it compiles and ends in `ret`.
    const a = std.testing.allocator;
    var func = ir.function.Function.init(a);
    defer func.deinit();
    const i32_t = try func.types.intern(.{ .int = .{ .signedness = .signed, .bits = 32 } });
...
test "Wimmer path: more than four live ints compiles with spilling" {
    ...
    func.setTerminator(e, .{ .ret = ir.function.Ret.one(s4) });
...
test "Wimmer path: an esi-homed boolean stages setcc through ebx (byte-addressability)" {
    // Force the icmp result to live in esi (which has no low byte): a and b occupy eax and ecx as
    // the compare operands, m1 occupies edx, so the boolean lands in the only remaining
    // allocatable register, esi. A direct `setcc esi` would wrongly encode DH (a latent bug this
    // fixes), so the emitter must stage the boolean through the byte-addressable ebx
    // (bl) and then move it to esi. This is a structural (disassembly) proof; full execution is
    // covered elsewhere.
    const a = std.testing.allocator;
    var func = ir.function.Function.init(a);
    defer func.deinit();
    ...
    func.setTerminator(e, .{ .ret = ir.function.Ret.one(s2) });
...
test "Wimmer path: a div whose divisor is homed in eax is copied out before the clobber" {
    ...
    // dividend, and idiv must read that copy, not eax. This is a structural (disassembly) proof;
    // a full execution differential is covered elsewhere.
    const a = std.testing.allocator;
    var func = ir.function.Function.init(a);
    defer func.deinit();
    const i32_t = try func.types.intern(.{ .int = .{ .signedness = .signed, .bits = 32 } });
    ...
    func.setTerminator(e, .{ .ret = ir.function.Ret.one(d) });
    ...
    // The miscompile signature: dividing by whatever now sits in eax (the dividend, after the clobber)
...
test "Wimmer path: a shift whose lhs is homed in ecx is copied out before the clobber" {
    ...
    // count, that is, shifts the count by itself. The guard must copy y out of ecx into the byte
    // scratch (ebx) before ecx is overwritten with the count, and the shift must read that copy.
    // This is a structural (disassembly) proof; a full execution differential is covered elsewhere.
    const a = std.testing.allocator;
    var func = ir.function.Function.init(a);
    defer func.deinit();
    const i32_t = try func.types.intern(.{ .int = .{ .signedness = .signed, .bits = 32 } });
...
test "codegen+disasm round-trip: an i8 store stages through ebx (byte-addressability)" {
    ...
    // be byte-addressable (esi and edi have no 8-bit form, so a source homed there would otherwise
    // misencode). Here the value is homed in eax, and the store still routes through ebx. This
    // confirms the byte-addressability staging, not just the [reg+0] store path in isolation.
    const a = std.testing.allocator;
    var func = ir.function.Function.init(a);
    defer func.deinit();
...
// --- Address fold under register pressure through the fold-aware Wimmer pipeline -------------
//
// The shared allocator is fold-blind: it reads only the raw IR operands. A foldable
// `p = arith_imm.add(base, imm); load/store(p)` leaves `p` (the add's result) dead once the memory op
// folds to `disp(base)`, but a fold-blind Wimmer compile would let `base` die at the add and reuse its
// register across the pressured region, then emit `disp(base)` reading the stale register (a trap
// found earlier on aarch64). `applyFoldRewriteX86` repoints each folded memory op's pointer to `base`
// and removes the dead adds before `wimmer.allocate`, so the allocator keeps `base` live to the memory
// op while the same analysis still drives the `disp(base)` emission. This test builds the
// fold-under-pressure shape, compiles it through the fold-aware Wimmer pipeline, asserts the fold
// actually fired in the emitted bytes, and (under qemu-i386 or an i386 host) asserts the result equals
// a hand-computed ground truth. A fold-blind allocation would read a stale base and diverge from that
// ground truth.

const fold_pressure = 8;

/// f(arg, cond): in the entry block, `buf1 = arg` and two dead adds off buf0 (`pl = buf0 + 4` = buf1's
/// address, `ps = buf0 + 8` = buf2's address), each used only by a successor memory op, so both fold.
/// On cond > 0, `then_b` first builds `fold_pressure` live temporaries (more than the four-register
/// pool, so a fold-blind allocator would have handed buf0's register to a temp by the time the folded
/// accesses run), then the folded store to `ps` and the folded load from `pl`. buf0 must stay live
/// across the whole pressured region for both folded accesses to read the right slot. Allocas are i32
/// (4 bytes), laid out back-to-back, so `buf0 + 4` = buf1 and `buf0 + 8` = buf2. Expected(cond>0) =
/// arg + cond + sum_{k=1..P}(cond + k); Expected(cond<=0) = cond.
...
/// accesses address off esp, so requiring base != esp isolates the folds. This is zero unless address
/// folding fired. It mirrors the `foldedMemOps` disassembly probe in `tests/addrfold.zig`, but works on
/// already-compiled bytes so it can inspect the Wimmer-fold output directly.
fn foldedMemOpsInCode(allocator: std.mem.Allocator, code: []const u8) !usize {
    ...
    while (lines.next()) |line| {
        ...
        if (std.mem.eql(u8, inner[0..base_end], "esp")) continue; // spill, alloca, or frame access, not a fold
...
test "Wimmer path: address fold under register pressure fires and computes the ground truth" {
    ...
    // survive as `[base + disp]` off a non-esp register. A zero here means the rewrite elided the fold,
    ...
    for (inputs) |args| {
        ...
        // then the P=fold_pressure temporaries (cond+k, for k=1..P) are summed in: arg + cond +
        // sum_{k=1..P}(cond + k). On cond <= 0 the else arm returns cond. This uses i32 wrapping, read
        // modulo 256.
